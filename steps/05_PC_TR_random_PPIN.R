...
for (percs in Global.percs) {
  ...
  for (rf in list.files(RANKING_DIR, pattern="Y_TR.*.Rdata")) {
    ...
    #  PV:  random percentage view la calcolo una sola volta, ?? uguale per tutte le f
    tA = Sys.time()
    ...
    RPF[[ "TR" ]][[ f_type ]][[ "R" ]] = matrix(data=c(0.0), nrow=N, ncol=length(percs))
    rownames(RPF[[ "TR" ]][[ f_type ]][[ "R" ]]) = as.character(1:N)
    colnames(RPF[[ "TR" ]][[ f_type ]][[ "R" ]]) = c(as.character(percs))

    RPF[[ "TR" ]][[ f_type ]][[ "P" ]] = matrix(data=c(0.0), nrow=N, ncol=length(percs))
    rownames(RPF[[ "TR" ]][[ f_type ]][[ "P" ]]) = as.character(1:N)
    colnames(RPF[[ "TR" ]][[ f_type ]][[ "P" ]]) = c(as.character(percs))

    RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]] = matrix(data=c(0.0), nrow=N, ncol=length(percs))
    rownames(RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]]) = as.character(1:N)
    colnames(RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]]) = c(as.character(percs))
    
    SUBG_DIR = ''
    if (SAVE_SUBGRAPHS) {
      for (ii in rownames(RPV[[ "TR" ]][[ f_type ]]))
        dir.create(file.path(CURRENT_DIR, "subgraphs", ii), recursive = T)
      SUBG_DIR = file.path(CURRENT_DIR, "subgraphs")
    }
    
    P = performance_PV(Y[[ 1 ]],                   # original random R2 
                       RPV[[ "TR" ]][[ f_type ]],  # induced  random PV
    ...
    RPF[[ "TR" ]][[ f_type ]][[ "R" ]] = P$R
    RPF[[ "TR" ]][[ f_type ]][[ "P" ]] = P$P
    RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]] = P$Fm
    RPF[[ "TR" ]][[ f_type ]][[ "CK" ]] = P$CK
    RPF[[ "TR" ]][[ f_type ]][[ "id_max" ]] = P$id_max
    
    getTiming(tA, Sys.time(), sprintf("performance_PV: %s", f_type))
    
    
    # Scrivo in output: RPV
    ...
    # Scrivi in output: RPF per ogni singola f
    ## Recall
    write.table(RPF[[ "TR" ]][[ f_type ]][[ "R" ]],          #  and the entire P matrix as well
                file=file.path(CURRENT_DIR,
                               "RPF",
                               sprintf("RPF_R.txt")),
                sep='\t',
    ...
    write.table(round(RPF[[ "TR" ]][[ f_type ]][[ "R" ]], digits=roundTo),  #  also the rounded version used for plots
                file=file.path(CURRENT_DIR, 
                               "RPF",
                               sprintf("RPF_R_rounded.txt")),
                sep='\t',
                quote=T)
    
    ## Precision
    write.table(RPF[[ "TR" ]][[ f_type ]][[ "P" ]],          #  and the entire P matrix as well
                file=file.path(CURRENT_DIR,
                               "RPF",
                               sprintf("RPF_P.txt")),
                sep='\t',
    ...
    write.table(round(RPF[[ "TR" ]][[ f_type ]][[ "P" ]],digits=roundTo),  #  also the rounded version used for plots
                file=file.path(CURRENT_DIR, 
                               "RPF",
                               sprintf("RPF_P_rounded.txt")),
                sep='\t',
                quote=T)
    
    ## F-measure
    write.table(RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]],          #  and the entire P matrix as well
                file=file.path(CURRENT_DIR,
                               "RPF",
                               sprintf("RPF_Fm.txt")),
                sep='\t',
    ...
    write.table(round(RPF[[ "TR" ]][[ f_type ]][[ "Fm" ]],digits=roundTo),  #  also the rounded version used for plots
                file=file.path(CURRENT_DIR, 
                               "RPF",
                               sprintf("RPF_Fm_rounded.txt")),
                sep='\t',
                quote=T)
