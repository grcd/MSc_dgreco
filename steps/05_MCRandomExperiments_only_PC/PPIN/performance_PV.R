...
#     * G è il grafo originario, che rendiamo ranked qui (solo nel caso dei random rankings)
...
source("steps/05_MCRandomExperiments_only_PC/PPIN/OS_match.R");

performance_PV <- function(R2, PV, G1, K, sigma_thr=0.2, id_to_index='', ft='', SUBG_DIR='', verbose=F) {
  
  if (ft == "incremental") {
    return(performance_PV_INC(R2, PV, G1, K, sigma_thr, id_to_index=id_to_index, SUBG_DIR=SUBG_DIR, verbose=verbose));
  } else if (ft == "decremental") {
    return(performance_PV_DEC(R2, PV, G1, K, sigma_thr, id_to_index=id_to_index, SUBG_DIR=SUBG_DIR, verbose=verbose));
  } else {
...
performance_PV_INC <- function(R2, PV, G1, K, sigma_thr=0.2, id_to_index='', SUBG_DIR='', verbose=F) {
  ...
  for (ii in 1:nrow(R2)) {  # le righe dei random rankings non hanno nomi
    
    f = as.character(ii)
    
    G = G1                 # come promesso, rendo il grafo ranked qui dentro
    E(G)$rank = R2[ii, ]
    ...
    for (pi in 1:ncol(PV)) {     # for every INCREMENTAL percentage rank view
      ...
      edges = E(G)[ which( E(G)$rank <= PV[ii, pi] ) ]
      ...
      if (SUBG_DIR != '') {
        ...
        write.graph(subg, 
                    file=file.path(SUBG_DIR, f, paste(f, "_RPV_", pi, ".gml", sep="")), 
                    format="gml")
      }    
...
performance_PV_DEC <- function(R2, PV, G1, K, sigma_thr=0.2, id_to_index='', SUBG_DIR='', verbose=F) {
