...
#  KB_NORM_MIN_LA.R
#
#  Calcola la kb_rec_min in forma matriciale (LA) corretta
...
# wf SIGNATURE
wf.NAME = "kb_rec_min"
wf.DESC = "For each undirected edge {u,v}, assigns weight(u,v) == MIN{rec(u,v), rec(v,u)} (LA version)."
wf.ARGS = c()
wf.MIXEDGES = TRUE
wf.VERBOSE = FALSE
...
kb_rec_min <- function(graph, K=KB.K, logfile="")
{
  # MATRIX COMPUTATION ------------------------------------------------------
  ...
  if (wf.VERBOSE)
    getTiming(tA0, Sys.time(), sprintf("kb_rec_min matrix computation"), file=logfile)
  ...
  graph=graph_from_adjacency_matrix(X_i_minus_1, mode="min", weighted=TRUE, diag=FALSE)
  
  E(graph)$measure = E(graph)$weight
  graph = remove.edge.attribute(graph, "weight")
  
  if (wf.VERBOSE)
    getTiming(tA, Sys.time(), "kb_rec_min re-assigning", file=logfile)
